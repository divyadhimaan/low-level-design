...
4. [start vs run · join · sleep · yield · interrupt](./concepts/concurrency/05-start-run-join-sleep-yield-interrupt.md) — `start()` vs directly calling `run()`; `join()`/`join(ms)` and why a timed join doesn't cancel the target; `sleep()` holds locks; `yield()` is an unreliable hint; cooperative cancellation via `interrupt()`/`isInterrupted()`/`Thread.interrupted()` and the restore-the-flag idiom.
